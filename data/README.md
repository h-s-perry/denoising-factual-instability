...
`fixtures/anchors.jsonl` contains four self-authored synthetic families (eight supported/refuted claims) under CC0-1.0. `fixtures/fixed-masks.json` uses a tiny synthetic offset tokenizer and deliberately splits every edited word into two pieces. `fixtures/saved-marginals.npz` contains synthetic seven-token posterior distributions and independently frozen expected statistics. `fixtures/llada-tokenizer-mask0.json` freezes the pinned real LLaDA tokenizer's claim IDs, offsets, word-piece mappings, prior-arm mask-0 geometry, targets, masked inputs, and work IDs for those same synthetic claims; its optional integration test runs cache-only and never loads model weights.
