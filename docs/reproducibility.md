...
Google Drive will be the durable cache for the A100 runner, but never a per-batch GPU hot-path filesystem. Publication is single-writer; concurrent writers are unsupported in v0.1.
...
The fixed throughput workload is at least 4,096 prior-only requests, five warm-up batches, and three full timed repetitions. Record end-to-end time, physical requests/second, forward count, peak VRAM, padding ratio, result-write time, and uploader stalls. No absolute speed or accepted batch-token default exists before that receipt.
